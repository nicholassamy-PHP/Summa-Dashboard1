    ...
    <div className={`card-premium border-l-4 ${accentColors[color]} p-8 group hover:border-opacity-100 transition-all`}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <p className="text-xs font-semibold text-gray-300 uppercase tracking-widest letter-spacing">{title}</p>
          <p className={`kpi-value mt-4 ${accentColors[color].split(' ')[0]}`}>{value}</p>
          {subtitle && (
            <p className="text-sm text-gray-300 mt-3 font-medium">{subtitle}</p>
          )}
        </div>
        {icon && (
